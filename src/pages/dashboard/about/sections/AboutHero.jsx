...
// Default data for About Hero
const DEFAULT_DATA = {
  title: "About Velvet Suites",
  subtitle: "A luxury boutique hotel in the heart of Kigali City",
...
export default function AboutHero({ data, onSave }) {
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      let url, method;
      if (sectionData.id) {
        url = `${API_URL}/about/hero/${sectionData.id}`;
        method = "POST";
        formData.append("_method", "PUT");
      } else {
        url = `${API_URL}/about/hero`;
        method = "POST";
      }
      ...
      if (result.success) {
        ...
        setError(result.message || "Error saving about hero");
      }
    } catch (err) {
      console.error("Save error:", err);
      setError(err.message || "Failed to save about hero");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {sectionData.id ? "Edit the about hero content" : "Create a new about hero"}
            </p>
          </div>
      ...
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <div className="relative h-64 overflow-hidden rounded-xl bg-gradient-to-r from-gray-800 to-gray-900">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white p-6">
            <h2 className="text-4xl font-bold mb-2">{sectionData.title || "About Velvet Suites"}</h2>
            <p className="text-xl text-white/80">{sectionData.subtitle || "A luxury boutique hotel in the heart of Kigali City"}</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1 text-xs text-amber-600">
              <li>• Upload a high-quality background image for the about hero</li>
              <li>• The title and subtitle should reflect your brand story</li>
