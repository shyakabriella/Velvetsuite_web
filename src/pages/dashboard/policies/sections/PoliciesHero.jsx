...
// Default data for Policies Hero
const DEFAULT_DATA = {
  title: "Policies & Practical Details",
...
export default function PoliciesHero({ data, onSave }) {
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      let url, method;
      if (sectionData.id) {
        url = `${API_URL}/policies/hero/${sectionData.id}`;
        method = "POST";
        formData.append("_method", "PUT");
      } else {
        url = `${API_URL}/policies/hero`;
        method = "POST";
      }
      ...
      if (result.success) {
        ...
        setError(result.message || "Error saving policies hero");
      }
    } catch (err) {
      console.error("Save error:", err);
      setError(err.message || "Failed to save policies hero");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Policies Hero Section</h2>
            <p className="text-sm text-gray-500">
              {sectionData.id ? "Edit the policies hero content" : "Create a new policies hero"}
            </p>
          </div>
      ...
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-white relative z-10">
            {sectionData.title || "Policies & Practical Details"}
          </h2>
        </div>
      </div>
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1 text-xs text-amber-600">
              <li>• Upload a background image for a more visually appealing hero section</li>
              <li>• The title should clearly identify the policies page</li>
